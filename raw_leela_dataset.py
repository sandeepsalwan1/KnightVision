                ...
                for member in members:
                    if member.isfile():
                        ...
                        for i in indices:
                            ...
                            #print(inputs[i].shape, policy[i].shape, orig_q[i].shape)
                            yield inputs[i], policy[i], orig_q[i]
dataset = LeelaDataset(chunk_dir="/Users/ralph/Data/lc0_tars")
dataloader = torch.utils.data.DataLoader(dataset, batch_size=100000)
it = iter(dataloader)

for i in range(100000):
    print("Iteration", i)
    inputs, policy, orig_q = next(it)
    torch.save({
       "inputs": inputs,
       "policy": policy,
       "q": orig_q,
    }, f"/Users/ralph/Data/lc0_filtered/file{i}.pt")
